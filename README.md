...
| URL | Descripción |
|---|---|
| http://3.132.203.79/health | Healthcheck (75% stable / 25% canary) |
| http://3.132.203.79:5000/health | Solo stable |
| http://3.132.203.79:5001/health | Solo canary |
| http://3.132.203.79/docs | Swagger general |
| http://3.132.203.79:5000/docs | Swagger stable |
| http://3.132.203.79:5001/docs | Swagger canary |

### Monitoreo de la estrategia
```bash
# Ver distribución del tráfico en tiempo real
for i in {1..20}; do
  curl -s http://3.132.203.79/health | python3 -c "import sys,json; d=json.load(sys.stdin); print(d['status'], d['version'])"
done
