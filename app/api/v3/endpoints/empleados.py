...
# Funcion para buscar empleados con filtros, orden y paginacion (novedad v3)
...
def listar_empleados(
    ...
    rol: str | None = Query(default=None, description="Filtra por rol (exacto)."),
    order_by: str = Query(default="cedula", pattern="^(cedula|nombre|rol)$"),
    order_dir: str = Query(default="asc", pattern="^(asc|desc)$"),
    skip: int = Query(default=0, ge=0),
    limit: int = Query(default=50, ge=1, le=200),
):
    empleados = crud_empleado.search_empleados(
        db,
        nombre=nombre,
        rol=rol,
        order_by=order_by,
