...
def test_actualizar_labor(client):
    ...
    response = client.patch("/api/v1/labores/P01", json={"precio": 75000.0})
    assert response.status_code == 200
...
def test_actualizar_labor_no_existe(client):
    response = client.patch("/api/v1/labores/X99", json={"nombre": "No existe"})
    assert response.status_code == 404


# Test para reemplazar una labor usando PUT
def test_reemplazar_labor(client):
    client.post(
        "/api/v1/labores/",
        json={
            "codigo_labor": "P01",
    ...
    response = client.put(
        "/api/v1/labores/P01",
        json={
            "nombre": "Poda Completa",
            "unidad_medida": "Metro",
            "precio": 80000.0,
            "observacion": "Reemplazada",
        },
    )
    assert response.status_code == 200
    assert response.json()["nombre"] == "Poda Completa"
    assert response.json()["unidad_medida"] == "Metro"
    assert response.json()["precio"] == 80000.0
    assert response.json()["observacion"] == "Reemplazada"


# Test para reemplazar una labor no existente
def test_reemplazar_labor_no_existe(client):
    response = client.put(
        "/api/v1/labores/X99",
        json={"nombre": "No existe", "unidad_medida": "Kg", "precio": 1000.0},
    )
    assert response.status_code == 404


# Test para reemplazar una labor con campos faltantes
def test_reemplazar_labor_campos_faltantes(client):
    client.post(
        "/api/v1/labores/",
        json={
            "codigo_labor": "P01",
    ...
    response = client.put("/api/v1/labores/P01", json={"nombre": "Solo nombre"})
