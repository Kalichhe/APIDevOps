...
def test_crear_registro_empleado_no_existe(client):
    ...
    response = client.post(
        "/api/v1/registrolabores/",
        json={
            "id": 1,
            "empleado_cedula": 999999999,
            "codigo_labor": "P01",
            "fecha": "2024-01-15",
            "cantidad": 2.5,
        },
...
def test_actualizar_registro(client):
    ...
    response = client.patch(
        f"/api/v1/registrolabores/{registro_id}", json={"cantidad": 5.0}
    )
    assert response.status_code == 200
...
# Test para reemplazar un registro usando PUT
def test_reemplazar_registro(client):
    crear_empleado_y_labor(client)
    crear = client.post("/api/v1/registrolabores/", json=REGISTRO)
    registro_id = crear.json()["id"]
    response = client.put(
        f"/api/v1/registrolabores/{registro_id}",
        json={
            "empleado_cedula": 123456789,
            "codigo_labor": "P01",
            "fecha": "2025-06-01",
            "cantidad": 10.0,
            "observacion": "Reemplazado",
        },
    )
    assert response.status_code == 200
    assert response.json()["cantidad"] == 10.0
    assert response.json()["fecha"] == "2025-06-01"
    assert response.json()["observacion"] == "Reemplazado"


# Test para reemplazar un registro no existente
def test_reemplazar_registro_no_existe(client):
    crear_empleado_y_labor(client)
    response = client.put(
        "/api/v1/registrolabores/999",
        json={
            "empleado_cedula": 123456789,
            "codigo_labor": "P01",
            "fecha": "2025-06-01",
...
# Test para reemplazar un registro con empleado no existente
def test_reemplazar_registro_empleado_no_existe(client):
    crear_empleado_y_labor(client)
    crear = client.post("/api/v1/registrolabores/", json=REGISTRO)
    registro_id = crear.json()["id"]
    response = client.put(
...
# Test para reemplazar un registro con labor no existente
def test_reemplazar_registro_labor_no_existe(client):
    crear_empleado_y_labor(client)
    crear = client.post("/api/v1/registrolabores/", json=REGISTRO)
    registro_id = crear.json()["id"]
    response = client.put(
...
# Test para reemplazar un registro con campos faltantes
def test_reemplazar_registro_campos_faltantes(client):
    crear_empleado_y_labor(client)
    crear = client.post("/api/v1/registrolabores/", json=REGISTRO)
    registro_id = crear.json()["id"]
    response = client.put(
        f"/api/v1/registrolabores/{registro_id}", json={"cantidad": 10.0}
    )
